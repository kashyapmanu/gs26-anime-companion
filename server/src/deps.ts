...
export function buildDeps(env: Env = loadEnv()): Deps {
  ...
  if (env.OPENROUTER_API_KEY && env.OPENROUTER_LLM_MODEL) {
    const openrouterClient = new OpenAI({ baseURL: env.OPENROUTER_BASE_URL, apiKey: env.OPENROUTER_API_KEY });
    llmProviders.push({ name: "openrouter", client: openrouterClient as any, model: env.OPENROUTER_LLM_MODEL });
  }

  const llm = new LLMProxy({ providers: llmProviders });
